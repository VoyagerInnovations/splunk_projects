...
echo "TIMESTAMP: $TS" >> $resultfile
echo "USERNAME = $uname" >> $resultfile
echo "TICKET = $ticket" >> $resultfile


case $action in 
	block_and_create_new_virtual_account)
    echo "ACTION = virtual_card_replacement" >> $resultfile
	echo "DETAILS = \"MIN: $min\"" >> $resultfile
        x=$(cat $logfile | grep -c "DONE BLOCKING AND CREATING NEW VIRTUAL ACCOUNT.")
	if [ $x -eq 0 ]; then
    ERROR=$(cat $logfile | egrep -i "ERROR|INFO|NOTICE")
        echo "STATUS = FAILED" >> $resultfile
        echo "$ERROR" >> $resultfile
	else
	echo "STATUS = SUCCESSFUL" >> $resultfile
	fi
	;;
	cancel_account)
    echo "ACTION = min_deregistration_from_account" >> $resultfile
	echo "DETAILS = \"MIN: $min\"" >> $resultfile
        ...
        ;;
	min_reassignment)
    echo "ACTION = min_reassignment" >> $resultfile
	echo "DETAILS = \"OLD MIN: $oldMin NEW MIN: $newMin\"" >> $resultfile
	...
	update_profile)
        case $updateProfile in 
	     tab_update_name)
             echo "ACTION = update_profile" >> $resultfile
	     ...
	     tab_update_birthday)
             echo "ACTION = update_profile" >> $resultfile
             echo "DETAILS = \"MIN: $min NEW BIRTHDAY: $updateBirthday\"" >> $resultfile
    ...
    echo "ACTION = downgrade_kyc1" >> $resultfile
        if [ $x -eq 0 ]; then
	echo "DETAILS = \"MIN: $min \"" >> $resultfile
        ERROR=$(cat $logfile | egrep -i "ERROR|INFO|NOTICE")
                echo "STATUS = FAILED" >> $resultfile
	...
	echo "DETAILS = \"MIN: $min $y\"" >> $resultfile
        echo "STATUS = SUCCESSFUL" >> $resultfile
        fi	
	;;
	rema_claim_transaction_update)
    echo "ACTION = rema_status_update" >> $resultfile
	echo "DETAILS = \"file: $file | SRCMIN: $srcMin RRN: $RRN \"" >> $resultfile
    ...
    echo "ACTION = batch_efs_closure" >> $resultfile
	echo "DETAILS = \"file: $file | Check output for list of blocked MINs\"" >> $resultfile
    ...
    echo "ACTION = batch_cms_closure" >> $resultfile
	echo "DETAILS = \"file: $file | Check output for list of blocked MINs\"" >> $resultfile
        ...
        ;;
	lift_dedup_account)
    echo "ACTION = lift_dedupped_accounts_in_efs" >> $resultfile
	x=$(cat $logfile | grep -c "transaction_reference_no")
        y=$(cat $logfile | grep "transaction_reference_no")
        if [ $x -eq 0 ]; then
        echo "DETAILS = \"MIN: $min \"" >> $resultfile
	...
	;;
	lift_closed_account)
    echo "ACTION = lift_closed_accounts_in_efs" >> $resultfile
	echo "DETAILS = \"file: $file | Check output for list of MINs\"" >> $resultfile
    ...
    echo "ACTION = lift_blacklisted_accounts_in_efs" >> $resultfile
	echo "DETAILS = \"file: $file | Check output for list of MINs\"" >> $resultfile
        x=$(cat $logfile | grep -c "Updated account_status to ACTIVE")
        if [ $x -eq 0 ]; then
        ERROR=$(cat $logfile | egrep -i "ERROR|INFO|NOTICE")
	...
	;;
    suspend_accounts)
    echo "ACTION = batch_efs_suspension" >> $resultfile
	echo "DETAILS = \"file: $file | Check output for list of blocked MINs\"" >> $resultfile
        x=$(cat $logfile | grep -c "not_activated")
        if [ $x -eq 0 ]; then
        ERROR=$(cat $logfile | egrep -i "ERROR|INFO|NOTICE")
    ...
    echo "ACTION = batch_cms_suspension" >> $resultfile
    echo "DETAILS = \"file: $file | Check MIN status using check_cms_status\"" >> $resultfile
        x=$(cat $logfile | grep -c "procedure successfully completed.")
        if [ $x -eq 0 ]; then
        ERROR=$(cat $logfile | egrep -i "ERROR|INFO|NOTICE")
                echo "STATUS = FAILED" >> $resultfile
    ...
    echo "ACTION = lift_suspended_accounts_in_efs" >> $resultfile
	echo "DETAILS = \"file: $file | Check output for list of MINs\"" >> $resultfile
        x=$(cat $logfile | grep -c "Updated account_status to ACTIVE")
        if [ $x -eq 0 ]; then
        ERROR=$(cat $logfile | egrep -i "ERROR|INFO|NOTICE")
